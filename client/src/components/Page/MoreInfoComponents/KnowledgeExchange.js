import React from "react";
import MoreInfo from "./../MoreInfo";

const KnowledgeExchange = () => {
  return (
    <>
      <div className="right-box">
        <div className="title">
          <h1>Wymiana wiedzy</h1>
        </div>
        <div className="scrollBox">
          <p>
            Dzięki wbudowanemu w aplikację portalowi społecznościowemu
            użytkownicy będą mogli w łatwy sposób wymieniać się wiedzą,
            doswiadczeniem i opiniami. Pozowli to poszczególnym użytkownikom na
            poszerzenie swojej wiedzy i skorzystanie z doświadczeń innych
            ogrodników, stanowiących społeczność gardenbook'a. Ta funkcjonalność
            nie jest jeszcze wdrożona.
          </p>
          <p className="slogan-green">
            Założ konto! Korzystaj z zasobów i narzędzi gardenbook'a.
          </p>
        </div>
      </div>
      <MoreInfo />
    </>
  );
};

export default KnowledgeExchange;
